...
public class KubeJSGatlingItem extends GatlingItem implements GunSkillItem {
    ...
    public KubeJSGatlingItem(Properties properties, KubeJSGatlingItemBuilder builder) {
        super(properties, builder.bonusDamage, builder.damageMultiplier, builder.fireDelayFractional, builder.inaccuracy, builder.enchantability);
        chanceFreeShot(builder.chanceFreeShot);
        headshotMult(builder.headshotMultiplier);
        projectiles(builder.projectiles);
    ...
    @Override protected void shoot(Level level, Player player, ItemStack gun, ItemStack ammo, IBullet bullet, boolean bulletFree) { GunProjectileSupport.shoot(this, level, player, gun, ammo, bullet, bulletFree, conversion, pierceCount, ammoConversions); }
