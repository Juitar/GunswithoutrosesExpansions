...
public class CeraunusStormBulletEntity extends BulletEntity implements DuskfallBulletDelegate {
    public CeraunusStormBulletEntity(EntityType<? extends BulletEntity> type, Level level) {
        super(type, level);
    }

    public CeraunusStormBulletEntity(Level level, LivingEntity shooter) {
        super(GWREEntities.CERAUNUS_STORM_BULLET.get(), shooter, level);
        setOwner(shooter);
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        if (!level().isClientSide) {
            ...
            if (target instanceof LivingEntity livingTarget && shooter instanceof LivingEntity livingShooter) {
                CeraunusBurstItem.applyStormBulletHit(livingTarget, livingShooter, getDamage());
            }
        }
    }
    ...
    protected void onHitBlock(BlockHitResult hit) {
        if (!level().isClientSide && getOwner() instanceof LivingEntity livingShooter) {
            CeraunusBurstItem.applyStormBulletBlockHit(level(), hit.getLocation(), livingShooter, getDamage());
        }
    }
